...
export default function CanvasCard({
  ...
  useEffect(() => {
    ...
    // Anchors
    const anchorLeft = new THREE.Mesh(
    ...
    const anchorRight = new THREE.Mesh(
      new THREE.SphereGeometry(0.05, 16, 16),
    ...
    // Create two straps (init empty)
    const strapLeftGeo = new LineGeometry();
    const strapLeft = new Line2(strapLeftGeo, strapMat);
    scene.add(strapLeft);

    const strapRightGeo = new LineGeometry();
    const strapRight = new Line2(strapRightGeo, strapMat);
    ...
    function onPointerUp(e) {
      isDragging = false;
      renderer.domElement.releasePointerCapture(e.pointerId || 1);
    }
    ...
    function animate() {
      ...
      if (!isDragging) {
        ...
        // tilt spring back
        const atx = -stiffness * (swingGroup.rotation.x - 0) - damping * velocity.tiltX;
        velocity.tiltX += atx * dt;
        swingGroup.rotation.x += velocity.tiltX * dt;

        const atz = -stiffness * (swingGroup.rotation.z - 0) - damping * velocity.tiltZ;
        velocity.tiltZ += atz * dt;
        swingGroup.rotation.z += velocity.tiltZ * dt;
      }

      // update straps with curve (sag)
  ...
  }, [imageUrl, width, height, cardSize, strapLength]);

  return (
    <div className="w-full h-full flex items-center justify-center bg-white">
      <div ref={mountRef} style={{ width: width + "px", height: height + "px" }}></div>
    </div>
  );
}
